...
# 你的训练配置是 JointPositionActionCfg(scale=0.5, use_default_offset=True)
# 仿真公式：q_des = q_default + 0.5 * action
# 真机首次测试不要直接用 0.5，本脚本默认从 0.15 开始。
TRAIN_ACTION_SCALE = 0.5

# 关节顺序必须和 Isaac Lab policy 的 action/joint 顺序一致。
JOINT_NAMES = [
...
Q_CENTER = 0.5 * (Q_LOW + Q_HIGH)
Q_HALF_RANGE = 0.5 * (Q_HIGH - Q_LOW) * SOFT_LIMIT_RATIO
...
def build_observation(state: RobotState, last_action: np.ndarray) -> np.ndarray:
    ...
    joint_pos_rel = state.joint_pos.astype(np.float32) - Q_DEFAULT
    joint_vel_rel = state.joint_vel.astype(np.float32)
    height_scan = np.zeros(HEIGHT_SCAN_DIM, dtype=np.float32)

    obs = np.concatenate([
        state.base_lin_vel.astype(np.float32),
        state.base_ang_vel.astype(np.float32),
        state.projected_gravity.astype(np.float32),
        state.cmd.astype(np.float32),
        joint_pos_rel,
        joint_vel_rel,
...
def action_to_q_des(action: np.ndarray, action_scale_real: float) -> np.ndarray:
    ...
    q = Q_DEFAULT + float(action_scale_real) * action
...
def run_dry(policy_path: str, device: str, action_scale_real: float, cmd: Tuple[float, float, float]) -> None:
    ...
    print("[DRY] 输入是假状态：水平站立、关节在默认站姿、height_scan=0")
    for i in range(20):
        ...
        for name, q in zip(JOINT_NAMES, q_des):
        ...
        while True:
            ...
            if state is None:
                if time.monotonic() - last_state_time > args.state_timeout:
                    ...
                    pkt = pack_command_packet(Q_DEFAULT, enable=0, mode=MODE_STAND, tick_ms=int(time.time() * 1000))
                    if not args.no_send:
                        ser.write(pkt)
                    last_state_time = time.monotonic()
                continue
            ...
            if not safe:
                ...
                q_des = limit_joint_rate(q_des, q_prev, args.max_dq)
                q_send = q_des
...
def parse_args():
    ...
    parser.add_argument("--action-scale", type=float, default=0.15, help="真机 action scale，首次建议 0.10~0.15，不要直接 0.5")
